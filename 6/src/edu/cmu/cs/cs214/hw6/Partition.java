...
public class Partition implements Iterable<File>, Serializable {
	...
	public Iterator<File> iterator() {
		File workerDataDir = new File(
				WorkerStorage.getDataDirectory(mWorkerName));
		File partitionDir = new File(workerDataDir, mPartitionName);

		if (!partitionDir.exists() || !partitionDir.isDirectory()) {
			throw new IllegalArgumentException(String.format(
					"Partition '%s' not found for worker '%s'.",
					mPartitionName, mWorkerName));
		} else if (!partitionDir.canRead()) {
			throw new IllegalStateException("Cannot read directory: "
					+ partitionDir.getPath());
		}
	...
	public String toString() {
		return String.format("<%s: partitionName=%s>",
				Partition.class.getSimpleName(), mPartitionName);
	}
	...
	public int hashCode() {
		...
		int result = 1;
		result = prime * result
				+ (mPartitionName == null ? 0 : mPartitionName.hashCode());
		return result;
	}
